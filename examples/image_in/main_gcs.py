#!/usr/bin/env python3
"""
ImageIn Filter Demo with GCS

This script demonstrates the ImageIn filter with Google Cloud Storage by:
1. Setting up a pipeline: ImageIn -> Util -> Webvis
2. Reading images from GCS bucket
3. Running the pipeline to display images in a web browser

Usage:
    python main_gcs.py [gcs_path]
    
    Examples:
        python main_gcs.py
        python main_gcs.py gs://my-bucket/images
        python main_gcs.py gs://protege-artifacts-development/labelled-data/demo_ocr/data
...
def main():
    """Run the ImageIn filter demo with GCS."""
    parser = argparse.ArgumentParser(description='ImageIn Filter Demo with GCS')
    ...
    print("ImageIn Filter Demo with GCS")
    print("============================")
