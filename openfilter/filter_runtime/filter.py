                    ...
                    + str(
                        (
                            _ := lambda cfg: (
                                ...
                                else cfg.__class__([_(v) for v in cfg])
    ...
    def normalize_config(
        ...
        if (exit_after := config.exit_after) is not None:
            if isinstance(exit_after, str):
                parse_date_and_or_time(exit_after[1:]) if exit_after.startswith(
                    "@"
                ) else parse_time_interval(exit_after)  # just validate
