...
```bash
pip install "openfilter[all] @ git+https://github.com/PlainsightAI/openfilter.git@v1.4.0"
```
...
both the version (for example `1.4.0`) and `latest`. Each installs `openfilter[extra]=={version}` from PyPI:
