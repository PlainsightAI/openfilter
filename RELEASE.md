...
- **CVE-2026-40962 is closed in both vendored ffmpeg copies**: `opencv-python-headless` moves to `~=5.0.0.93` and `av` (PyAV) to `~=17.1.0`, and both linux x86_64 wheels now bundle ffmpeg 8.1.1 (`libavcodec.so.62.28.101`, `libavutil.so.60.26.101`) where the previous pins bundled the ffmpeg 8.0 series. That closes the integer overflow and out-of-bounds write reachable through malformed CENC subsample data in the MOV demuxer, which upstream fixed in ffmpeg 8.1. This release does not close CVE-2026-8461, the heap out-of-bounds write in the MagicYUV decoder: upstream fixed that one in ffmpeg 8.1.2 and the wheels shipped here carry 8.1.1, so pipelines that decode untrusted MagicYUV content in AVI, MKV, or MOV containers remain exposed until wheels built against ffmpeg 8.1.2 or later are available.

### Added

- **Snapshot payload REST endpoint for Webvis**: an opt-in `GET /snapshot-payload` (and `GET /{topic}/snapshot-payload`) route that returns the latest frame as a single JSON document carrying `topic`, `timestamp`, `width`, `height`, `format`, `metadata`, and the JPEG image base64-encoded under `image`, so clients that poll over plain HTTP no longer need to hold the MJPEG stream or the SSE `/data` connection open. The route is registered only when `enable_snapshot_payload` is set (also via `FILTER_ENABLE_SNAPSHOT_PAYLOAD`), default off; it answers 404 while no frame has arrived yet and 500 if JPEG encoding fails. Topic resolution and the flat vs. combined multi-topic metadata shape are shared with the SSE `/data` endpoint through a new `get_metadata_for_topic` helper, so a bare `/snapshot-payload` on a multi-topic pipeline returns the combined metadata for all topics alongside the image of the first active topic. Two behavioral notes: `Webvis.process` now records the latest frame and its metadata on every frame instead of only when the stream queue is empty, so the snapshot reflects the freshest frame rather than the last one that made it into the queue; and while the endpoint is enabled it shadows a pipeline topic literally named `snapshot-payload`. Access to `streams`, `latest_frames`, and `current_data` is now guarded by a re-entrant lock.

### Changed

- **`opentelemetry-resourcedetector-gcp` moved to a stable-only range**: The pin went from `~=1.12.0a0` to `~=1.14.0`. The old range expands to `>=1.12.0a0,<1.13.dev0`, and the single version inside it, `1.12.0a0`, is a pre-release. `uv` only accepts a pre-release for a dependency it pulls in indirectly when that package has never published a stable version, so installing OpenFilter from a package index with `uv` failed to resolve unless the caller passed `--prerelease=allow`. `pip` applies a looser rule and installed the same range without complaint, so this only affected `uv` users. `1.14.0` is the first usable stable release of the package (`1.13.0` was yanked upstream), so `uv add openfilter` and `uv pip install openfilter` now resolve with no extra flags. The two remaining pre-release pins, `opentelemetry-exporter-gcp-monitoring~=1.12.0a0` and `opentelemetry-semantic-conventions~=0.65b0`, are deliberately left alone: neither package has ever published a stable release, which is exactly the condition under which `uv` accepts a pre-release.
- **Webvis uvicorn access logging disabled by default**: Uvicorn access logging (previously always enabled) is now off by default to prevent verbose request logging from cluttering logs during high-frequency client polling of the `/snapshot-payload` endpoints, controllable via the new `access_log` parameter and the `FILTER_ACCESS_LOG` environment variable.
- **`opencv-python-headless` major version bump, from `~=4.13.0` to `~=5.0.0.93`**: this is a core dependency rather than an extra, so every OpenFilter install takes it, and opencv 5 is a major release that removes API surface 4.x shipped. OpenFilter's own runtime and bundled filters reference none of the removed symbols and needed no changes, but downstream filters can break at import or on first call. The removals most likely to bite are `cv2.CascadeClassifier`, the `cv2.dnn` model readers `readNetFromCaffe`, `readNetFromDarknet`, and `readNetFromTorch`, and the `cv2.AKAZE_create`, `cv2.KAZE`, and `cv2.BRISK` feature detectors. The bundled cascade data is gone as well: `cv2.data.haarcascades` still resolves to a directory path, but that directory is empty in 5.0.0.93 where 4.13 installed 17 `haarcascade_*.xml` files. Any filter doing Haar cascade detection, loading a Caffe, Darknet, or Torch model through the legacy dnn readers, or building KAZE, AKAZE, or BRISK descriptors has to be ported before it will run on this release. `readNetFromONNX`, `readNetFromTensorflow`, `readNetFromTFLite`, `SIFT`, and `ORB` are unaffected.
- **`av` (PyAV) major version bump, from `~=16.1.0` to `~=17.1.0`**: `av` is used by the `video_out` encode path only, through a single import in `openfilter/filter_runtime/filters/video_out.py`, and it ships in the `video_out` and `all` extras, so installs that do not pull those extras are unaffected. PyAV vendors its own ffmpeg build, and the `17.1.0` linux x86_64 wheel bundles ffmpeg 8.1.1 where av 16.1.0 and 17.0.0 bundled ffmpeg 8.0, so the encode path moves onto the ffmpeg 8.1 series. `17.1.0` is the newest av release that still supports Python 3.10 (av 18 requires Python >= 3.11), so it is the ceiling for this cut.
- Refreshed the dependency floors to current releases: `openlineage-python~=1.51.0`, `requests~=2.34.2`, the OpenTelemetry stack to `~=1.44.0` (`semantic-conventions~=0.65b0`, GCP exporters `~=1.12.0a0`), `pydantic~=2.13.4`, `fastapi~=0.139.2`, `uvicorn~=0.51.0`, `python-multipart~=0.0.32`, `google-cloud-storage~=3.13.0`, `google-cloud-monitoring~=2.31.0`, and the `dev` tooling (`setuptools~=83.0.0`, `pytest~=9.1.1`, `build~=1.5.0`, `docker~=7.2.0`, `wheel~=0.47.0`, `jq~=1.12.0`, `pytest-cov~=7.1.0`). `numpy` is deliberately held back at `~=2.2.6` (numpy 2.5 requires Python >= 3.12), preserving the Python 3.10/3.11 support OpenFilter still guarantees. No filter code changes were required.

### Fixed

- **`lin` size suffix now selects bilinear interpolation instead of nearest-neighbour**: The size spec accepted by `VideoIn` (`resize` / `maxsize`) and by the `Util` filter's `resize` / `maxsize` / `minsize` xforms documents three interpolation suffixes, `near`, `lin` and `cub`, and the parser has always accepted all three. The ternary chain mapping the parsed suffix to an OpenCV constant only handled `N` and `C` explicitly and returned `cv2.INTER_NEAREST` from its final else branch, so `L` fell through to nearest-neighbour and `cv2.INTER_LINEAR` appeared nowhere in the shipped package. The two call sites are fixed in the shape each one needs. In `video_in.py` that else branch now returns `cv2.INTER_LINEAR` outright, which is safe because the ternary normalizes the suffix itself with `interp.upper()[:1]` and the size regex only admits `n`/`near`, `l`/`lin` and `c`/`cub`, so `L` is the only value that can reach the branch. In `util.py` the chain instead gains an explicit `cv2.INTER_LINEAR if interp == 'L'` step and keeps `cv2.INTER_NEAREST` as the final fallback, because xforms supplied in dict form bypass the string parser that upper-cases and truncates the suffix, so an unnormalized `n`, a spelled-out `near` or a typo would otherwise select bilinear silently. Both now give the documented mapping of `near`/unset to `INTER_NEAREST`, `cub` to `INTER_CUBIC` and `lin` to `INTER_LINEAR`. **This is a behaviour change**: pipelines already passing `lin` (for example `resize 640x480lin`) are silently getting nearest-neighbour output today and will get bilinear output after this fix, which changes resampled pixel values and may shift downstream detection or OCR results. Pipelines relying on the old output should pin `near` explicitly.
- **Per-GPU system metrics emitted as OTel counters instead of gauges**: `OpenTelemetryClient` classified gauge-vs-counter by membership in a static `_GAUGE_METRICS` set, but the per-GPU series (`gpuN`, `gpuN_mem`) are emitted with dynamic names, so they fell through to counters and accumulated a running sum, and the reported "GPU Utilization" metric ramped past 100%. Added name-based classification so per-GPU utilization and memory series (`gpuN` / `gpuN_mem`) are emitted as gauges alongside the static set. Genuine counters and the `gpu_usage_percent` / `gpu_accessible` health gauges are unchanged; this also corrects the data type of the `gpuN_mem` series.
