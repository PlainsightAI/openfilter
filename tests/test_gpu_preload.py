...
from openfilter.filter_runtime.gpu import (
    preload_gpu_libs as _preload_gpu_libs,
    _preloaded_gpu_libs,
)
...
class TestPreloadGpuLibs:
    ...
    def test_handles_multiple_paths(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("OPENFILTER_APPEND_LD_LIBRARY_PATH", f"{dir1}:{dir2}")
...
class TestPreloadRunsBeforeApplyEnvVars:
    ...
    def test_preload_runs_before_apply_env_vars(self):
        ...
        preload_match = re.search(
            r"^(?!.*\bdef\b).*preload_gpu_libs\(\)", source, re.MULTILINE
        )
        apply_match = re.search(
            r"^(?!.*\bdef\b).*_apply_append_env_vars\(\)", source, re.MULTILINE
        )
        assert preload_match is not None, (
            "preload_gpu_libs() module-level call not found"
        )
        assert apply_match is not None, (
            "_apply_append_env_vars() module-level call not found"
        )
        assert preload_match.start() < apply_match.start(), (
            "preload_gpu_libs() must be called before _apply_append_env_vars()"
        )
